...
class BaseModel:
    ...
    def save(self):
        ...
        if not table_path.exists():
            table_path.touch()#criando o arquivo
        with open(table_path, 'a') as arq: #modo 'a' escreve mais oq ja tinha
            arq.write("|".join(list(map(str, self.__dict__.values()))))#programação funcional
            arq.write(' \n')
    ...
    def get(cls):
        ...
        if not table_path.exists():
            table_path.touch()
        with open(table_path, 'r') as arq: #modo 'r' de leitura
            x = arq.readlines()
        results = []
